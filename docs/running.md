...
For real use, configure provider environment variables below, then run `pnpm exec vp run web --live`. Enter your brief in the browser and click **Start exploration**. The model, Jev availability and session compute limits are displayed before sending anything. The optional `--brief "Your brief"` flag creates a session ahead of time; click **Explore** to run it. `--live --resume` reopens the latest browser session. The built equivalent is `node dist/src/web/main.js --live`. Starting the browser does not make a paid call. Subsequent answers and steering can continue work until paused. **New session** stops the previous worker and starts a different brief. Previous data is preserved, but the browser currently resumes only the latest session. GRILL_WEB_PORT optionally selects a fixed local port.

The workspace keeps all decisions in a compact queue and opens the selected decision as a side-by-side comparison. Choosing an option saves it directly and moves to the next available question. It also supports branch findings and provenance, deferred decisions, reopening, a connected dependency graph with zoom and panning, constraint input and Markdown export. A live event stream reflects changes from the engine and adapters. Draft answers survive background updates. Free text on a question is an explicit alternative; multi-question agent interpretations appear as a review proposal requiring acceptance.
