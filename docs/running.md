...
For a real session, configure provider environment variables below, then run `pnpm exec vp run web --live --brief "Your brief"`. `--live --resume` reopens the latest browser session. The built equivalent is `node dist/src/web/main.js`. Starting the browser does not make a paid call. Click Explore to enable bounded work; subsequent answers and steering can continue it until paused. GRILL_WEB_PORT optionally selects a fixed local port.
